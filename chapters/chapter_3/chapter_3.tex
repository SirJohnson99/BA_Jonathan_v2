...
Zum Anregen der Platte werden Körperschallwandler der Firma Dayton Audio verwendet \cite{Visaton}. Die Abbildung \ref{fig:Exciter} zeigt die Vorder- und Rückseite des verwendeten Exciters, an der Rückseite des Exciters ist die Kontaktfläche, welche mit einem doppelseitigen Klebeband von 3M versehen ist.


\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{pictures/Exciter.png}
	\caption{Dayton Audio Exciter \cite{DaytonAudio2024}}
	\label{fig:Exciter}
\end{figure}

Das Modell ist der Typ DAEX32EP-4, welcher mit einer Leistung von \SI{40}{\watt} RMS bei einer Impedanz von 4\,$\Omega$ zu den leistungsstärkeren Modellen seiner Klasse zählt \cite{DaytonAudio2024}. Dieser Exciter wird mithilfe einer Klebefläche, welche einen Innendurchmesser von \SI{21,75}{\milli\meter} und einen Außendurchmesser von \SI{41,75}{\milli\meter} hat, an die vorgesehene Platte geklebt. Damit wird eine schlüssige Kraftübertragung von Exciter auf Platte gewährleistet. Der Frequenzverlauf des Exciters wurde vom Hersteller vermessen, dabei wurde eine ungefähr \SI{30}{\centi\meter} $\times$ \SI{30}{\centi\meter} und \SI{1,3}{\centi\meter} starke Sandwichplatte mit Schaumkern mit einem außermittig platziertem Exciter in einem Freifeldraum angeregt. Der SPL-Verlauf über der Frequenz ist dabei relativ linear im Bereich \SIrange{70}{5000}{\hertz} bei einem SPL von ca.\ \SI{83}{\deci\bel}, ab \SIrange{5000}{20000}{\hertz} fällt der SPL auf ca.\ \SI{68}{\deci\bel} ab.
...
Als DML-Material wurden zwei Platten in Sandwichbauweise der Firma 3A Composites zur Verfügung gestellt. Die beiden Platten haben eine Größe von \SI{1}{\meter} $\times$ \SI{0,7}{\meter} und unterscheiden sich im Wesentlichen in ihrer Stärke und dem Trägermaterial.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{pictures/KAPA_Vergleich.png}
...
Bei der eingesetzten Stereoanlage handelt es sich um die WPA-600 Pro Leistungsendstufe der Firma Wangine Electronics Co., welche als Verstärker zur Ansteuerung der verwendeten Körperschallwandler dient \cite{WangineWPA2024}.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{pictures/Stereo_EQ.png}
...
Die Entscheidungsfindung erfolgt auf Grundlage einer technischen Bewertung der möglichen Systemkonzepte. Dabei werden sowohl die akustischen und konstruktiven Anforderungen als auch die in Kapitel 3.1 beschriebenen Rahmenbedingungen berücksichtigt. Auf diese Weise wird eine nachvollziehbare Auswahl des für diese Arbeit geeigneten Systemkonzepts ermöglicht.
...
Während passive Weichen das Signal nach dem Verstärker in die verschiedenen Frequenzbereiche trennen, ermöglicht ein DSP die digitale Signalverarbeitung in Echtzeit und eine variable Trennfrequenz. Ein DSP kann neben der Frequenztrennung auch als Equalizer verwendet werden, was insbesondere bei DML-Systemen von Vorteil sein kann. 
...
Es ist zu erkennen, dass die ausgerechnete Biegesteifigkeit der Sandwichplatten von KAPA Line wesentlich größer ist als von KAPA Graph. Das liegt vor allem an der dreimal höheren Plattenstärke der KAPA Line Platte. Unter Berücksichtigung der Biegesteifigkeit und der zugehörigen Flächenmasse, welche aus den Datenblättern entnommen werden kann, bietet sich die Sandwichplatte aus KAPA Line besser als DML Material. Diese Annahme ist allein auf die Materialparameter im Vergleich zurückzuführen, da KAPA Line im Vergleich eine mehr als zehnmal so große Biegesteifigkeit aufweist bei einem Flächengewicht, welches gerade einmal 1,45 mal so groß ist wie KAPA Graph.

Nach der Auswahl des geeigneten Plattenmaterials erfolgt die Festlegung der geometrischen Abmessungen. Die Plattengröße stellt dabei einen wesentlichen Einflussparameter dar, da sie die Ausbildung der Eigenmoden und damit die resultierende modale Verteilung im Frequenzbereich beeinflusst. Da der DML-Lautsprecher im Rahmen dieser Arbeit als Fullrange-System ausgelegt wird, ist die Geometrie so zu wählen, dass eine möglichst ausgewogene Wiedergabe im Tief-, Mittel- und Hochtonbereich erreicht wird.

Neben den akustischen Anforderungen ist auch die geplante Nutzung der Plattenvorderseite als Informationsfläche zu berücksichtigen. Hierfür sollen Plakate im DIN-Format auf der Vorderseite angebracht werden. Da DIN-Formate ein Seitenverhältnis von näherungsweise 1:1,414 aufweisen, erscheint eine quadratische Plattengeometrie für diesen Anwendungsfall weniger geeignet.

Aus diesen Gründen wird für die Auslegung der Platte ein rechteckiges Seitenverhältnis gewählt. Als Grundlage dient hierbei das Verhältnis 1:1,618, welches dem Goldenen Schnitt $\varphi$ entspricht. Dadurch wird eine nichtquadratische Plattenform gewählt, die sowohl gestalterische Anforderungen als auch die akustische Auslegung des DML-Systems berücksichtigt.

Der Goldene Schnitt wird vor allem in der DML-DIY-Community empfohlen und ist für gute akustische Eigenschaften bekannt.
Gleiche Seitenlängen können Modenüberlagerungen und ausgeprägte Resonanzspitzen begünstigen, aus diesem Grund wird ein quadratisches Seitenverhältnis vermieden. Daher wird mit dem Goldenen Schnitt $\varphi$ ein irrationales Seitenverhältnis gewählt, um eine gleichmäßigere Modenverteilung zu unterstützen.

Die Plattenabmessungen werden auf Grundlage des Seitenverhältnisses 1:1,618 sowie der angestrebten Grundfrequenz zur Tieftonwiedergabe festgelegt. Für rechteckige Platten lässt sich diese näherungsweise mit folgender Gleichung bestimmen:
...
Dementsprechend ist eine Seitenlänge für den Bau eines DML aus KAPA Line Sandwichplatten von mindestens \SI{20}{\centi\meter} nötig, um eine Grundfrequenz von \SI{20}{\hertz} gut wiedergeben zu können. Da die KAPA Platten in einer Größe von \SI{1}{\meter} $\times$ \SI{0,7}{\meter} geliefert werden, wurde die Platte mit den Seitenlängen \SI{0,7}{\meter} $\times$ \SI{0,44}{\meter} gewählt, da diese das Seitenverhältnis erfüllen und die Anfertigung effektiv gestaltet.
...
Die Randbedingungen eines DML-Panels haben einen signifikanten Einfluss auf dessen vibroakustisches Verhalten. Die Art der Lagerung bestimmt maßgeblich die Modenstruktur, die Eigenfrequenzen sowie die Abstrahlcharakteristik des Systems \cite{Cremer1996}. In der Fachliteratur werden sechs grundlegende Lagerungskonzepte unterschieden:
...
Eine vergleichende Übersicht der Vor- und Nachteile aller Lagerungsarten ist in Tabelle \ref{tab:lagerungsarten} dargestellt. Anhand der Tabelle ist zu erkennen, dass für diese Arbeit nur zwei der sechs Lagerungsarten eine sinnvolle Umsetzung ermöglichen: die frei schwebende Lagerung sowie die fest eingespannte Lagerung.

Im Rahmen dieser Arbeit wird die frei schwebende Lagerung untersucht, bei der das Panel an den beiden Eckpunkten einer Seite mittels Fäden aufgehängt wird. Diese Konfiguration zeichnet sich durch minimale externe Dämpfungseinflüsse aus und ermöglicht eine weitgehend ungestörte Ausbildung der Plattenmoden, da keine signifikanten Randreflexionen oder Energiedissipation an den Lagerstellen auftreten \cite{Moeser2010}. Die geringe Dissipation begünstigt insbesondere die Tieftonwiedergabe. Als nachteilig erweisen sich jedoch die vergleichsweise geringe Abstrahleffizienz sowie mögliche Pendelbewegungen des Panels bei tieffrequenter Anregung.

Im Gegensatz dazu steht die fest eingespannte Lagerung, bei der das Panel in einen starren Rahmen eingefasst wird. Diese Konfiguration führt zu einer erhöhten Abstrahleffizienz, da die Plattenränder als Knotenlinien fungieren und somit eine effektivere Schallabstrahlung ermöglichen. Dem gegenüber steht die Unterdrückung der Randmoden, wodurch die modale Gleichverteilung beeinträchtigt und die Tieftonwiedergabe reduziert wird. Darüber hinaus erfordert diese Lagerungsart einen erhöhten konstruktiven Aufwand.


\begin{table}[htbp]
\centering
\caption{Vor- und Nachteile verschiedener Lagerungsarten}
\label{tab:lagerungsarten}
\footnotesize
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\textwidth}{|X|X|X|}
\hline
\textbf{Lagerungsart} & \textbf{Vorteile} & \textbf{Nachteile} \\
\hline
Frei schwebend &
\vspace{-2mm}
\begin{itemize}[leftmargin=*, nosep, topsep=0pt, parsep=0pt]
    \item Minimale äußere Dämpfung
    \item Gleichmäßige Modenverteilung
    \item Keine Randeffekte
    \item Gute Tieftonwiedergabe
\end{itemize}
\vspace{-2mm}
&
\vspace{-2mm}
\begin{itemize}[leftmargin=*, nosep, topsep=0pt, parsep=0pt]
    \item Niedrige Abstrahleffizienz
    \item Pendeln bei tiefen Frequenzen
    \item Geringe mechanische Stabilität
    \item Praktisch schwer integrierbar
\end{itemize}
\vspace{-2mm} \\
\hline
Gelenkig gelagert &
\vspace{-2mm}
\begin{itemize}[leftmargin=*, nosep, topsep=0pt, parsep=0pt]
    \item Definierte Randbedingungen
    \item Gute Modellierbarkeit (FEM)
    \item Höhere Stabilität
    \item Reproduzierbare Modenstruktur
\end{itemize}
\vspace{-2mm}
&
\vspace{-2mm}
\begin{itemize}[leftmargin=*, nosep, topsep=0pt, parsep=0pt]
    \item Randreflexionen möglich
    \item Konstruktiv aufwendiger
    \item Beeinflussung tieffrequenter Moden
    \item Potentielle Auslöschungen
\end{itemize}
\vspace{-2mm} \\
\hline
Fest eingespannt &
\vspace{-2mm}
\begin{itemize}[leftmargin=*, nosep, topsep=0pt, parsep=0pt]
    \item Hohe mechanische Stabilität
    \item Klar definierte Randbedingungen
    \item Robust gegen äußere Einflüsse
    \item Langzeitstabil
\end{itemize}
\vspace{-2mm}
&
\vspace{-2mm}
\begin{itemize}[leftmargin=*, nosep, topsep=0pt, parsep=0pt]
    \item Starke Randdämpfung
    \item Verschiebung der Eigenfrequenzen
    \item Schlechtere Tieftonwiedergabe
    \item Ungleichmäßige Modendichte
\end{itemize}
\vspace{-2mm} \\
\hline
...
\begin{itemize}[leftmargin=*, nosep, topsep=0pt, parsep=0pt]
    \item Praxistauglich
    \item Dämpft störende Randmoden
    \item Kein Pendeln
    \item Kompakte Bauform
\end{itemize}
\vspace{-2mm}
&
\vspace{-2mm}
\begin{itemize}[leftmargin=*, nosep, topsep=0pt, parsep=0pt]
    \item Beeinflussung des Frequenzgangs
    \item Materialabhängiges Verhalten
    \item Möglicher Tieftonverlust
    \item Alterung des Dämpfungsmaterials
\end{itemize}
\vspace{-2mm} \\
\hline
Punktuelle Auflage &
...
\begin{itemize}[leftmargin=*, nosep, topsep=0pt, parsep=0pt]
    ...
    \item Flexible Positionierung
    \item Geringe Modenbeeinflussung
    \item Nahezu freie Schwingung
\end{itemize}
\vspace{-2mm}
&
\vspace{-2mm}
\begin{itemize}[leftmargin=*, nosep, topsep=0pt, parsep=0pt]
    \item Lokale Resonanzen
    \item Modenauslöschungen an Auflagepunkten
    \item Ungleichmäßige Krafteinleitung
    \item Kritische Punktauswahl nötig
\end{itemize}
\vspace{-2mm} \\
\hline
Randdämpfung &
\vspace{-2mm}
\begin{itemize}[leftmargin=*, nosep, topsep=0pt, parsep=0pt]
    \item Reduktion stehender Wellen
    \item Glatterer Frequenzgang
    \item Weniger Klangverfärbung
    \item Gezielte Modenkontrolle
\end{itemize}
\vspace{-2mm}
&
\vspace{-2mm}
\begin{itemize}[leftmargin=*, nosep, topsep=0pt, parsep=0pt]
    \item Geringerer Gesamtwirkungsgrad
    \item Tieftonverluste
    \item Materialabhängiger Klangcharakter
    \item Zusätzlicher Konstruktionsaufwand
\end{itemize}
\vspace{-2mm} \\
\hline
\end{tabularx}
...
Die Anzahl der Exciter hat sowohl auf die Gesamtimpedanz des Systems als auch auf die Abstrahlleistung Einfluss \cite{Bai2001}.

Die Verwendung eines einzelnen Exciters pro Platte ist die einfachste Umsetzung. Die Impedanz des Systems entspricht der des Exciters von 4\,$\Omega$ \cite{DaytonAudio2024}, was dem Anhang A zu entnehmen ist. Die verwendete Stereoanlage kann Lautsprecher von 4 bis 16\,$\Omega$ versorgen, siehe Anhang B. Dennoch sollte berücksichtigt werden, dass die Exciter bei voller Leistung auf Dauer beschädigt werden können. Zudem kann die Platzierung des Exciters ungünstig für die Modenanregung sein. 
...
Die KAPA Line Sandwichplatten werden eine Größe von \SI{0,7}{\meter} $\times$ \SI{0,44}{\meter} haben, da diese Seitenlängen in das Verhältnis passen und einfach herzustellen sind. Damit sind diese größer als im Graphen \ref{fig:seitenlaenge_frequenz} aufgezeigt. Diese Größe wurde aus dem Grund gewählt, um eine gute Tieftonwiedergabe zu gewährleisten und damit die Exciter nicht zu nahe aneinander platziert werden. 
...
Ziel der Simulation ist es, den Zeit- und Ressourcenaufwand für experimentelle Untersuchungen für die ideale Exciter-Position zu reduzieren. Die Simulation dient dazu, ideale Positionen der Exciter auf der Platte anhand einer Modalanalyse zu bestimmen \cite{Moeser2020}. 
...
Das Programm COMSOL Multiphysics der Firma COMSOL ist eine universell einsetzbare Simulationssoftware für einen Großteil der Bereiche des Ingenieurwesens. Die Software bietet vollständig gekoppelte Einzel- und Multiphysik-Modellierungsfunktionen und ist dabei strukturiert und benutzerfreundlich aufgebaut. Dabei umfasst COMSOL in seiner Produktpalette eine große Anzahl von Fachgebieten wie Elektromagnetik-Module, Strömungs- und Wärmetransport-Module, Strukturmechanik- und Akustik-Module und weitere. In dieser Arbeit wird das Strukturmechanik-Modul verwendet, um die modalen Eigenschaften der Platte zu untersuchen. Der Modellaufbau in COMSOL erfolgt über den Model Builder, in dem zuerst die globalen Definitionen festgelegt werden. Darunter zählen Parameter und Variablen, welche in der Simulation verwendet werden. Danach werden die Komponenten aufgebaut, dazu gehört 2D- oder 3D-Geometrie, welche im Grafikfenster dargestellt wird. Die Geometrie stellt das Grundgerüst der Simulation dar. In den folgenden Schritten werden Materialien, welche entweder aus der COMSOL-Materialbibliothek stammen oder eigenständig eingetragen werden, verwendet und der Geometrie zugewiesen. Nach der Zuweisung der Materialien wird die Physik in die Simulation hinzugefügt. In COMSOL können Simulationsmodelle flexibel angepasst werden. Werden mehrere physikalische Teilmodelle berücksichtigt, ist eine Multiphysik-Kopplung erforderlich, um deren Wechselwirkungen innerhalb der Simulation abzubilden. Nachdem Geometrie, Materialeigenschaften und physikalische Randbedingungen definiert wurden, wird das Modell vernetzt. Dabei wird die Geometrie durch ein Mesh in endlich viele kleine Elemente unterteilt, welches die Grundlage der FEM-Berechnung bildet. Anschließend kann die jeweilige Studie berechnet werden. Je nach Zielsetzung lassen sich unterschiedliche Studientypen hinzufügen oder entfernen.

Im Rahmen dieser Arbeit wird die Strukturschwingung einer DML-Platte rechnerisch untersucht. Ziel der Simulation ist es, die Eigenmoden der Platte sowie die zugehörigen Verschiebungsformen zu bestimmen und visuell darzustellen. Die Beschreibung des strukturmechanischen Verhaltens erfolgt auf Grundlage der linearisierten Elastizitätstheorie. Dabei wird der Zusammenhang zwischen Spannungen und Dehnungen über das verallgemeinerte Hooke’sche Gesetz abgebildet. Für die Berechnung wird die dynamische Gleichgewichtsbeziehung der Struktur gelöst, wodurch die modalen Verformungen der Platte im betrachteten Frequenzbereich analysiert werden können.



\subsection{Modell-Aufbau und Simulationen}

Bei der Simulation zur Entwicklung des DML wurde sich für eine Eigenfrequenzanalyse der verwendeten Platten und gegen die Simulation eines Exciters als Federmassesystem, welches die Platte anregt und der abgestrahlte Schalldruck berechnet wird, entschieden. Ausschlaggebend für diese Entscheidung ist der Unterschied in der Komplexität des Modellaufbaus, da eine Eigenfrequenzanalyse in der Komplexität und Rechenleistung effizienter ist.

Im Rahmen der Simulation wurden zwei KAPA Line Platten mit den Größen \SI{1}{\meter} $\times$ \SI{0,7}{\meter} und \SI{0,7}{\meter} $\times$ \SI{0,44}{\meter} mit der Stärke \SI{15}{\milli\meter} auf deren Eigenfrequenzen untersucht. Dabei wurden die Plattenmaße und die Eigenschaften des Materials, welche dem Datenblatt im Anhang C entnommen werden können, in dem Model Builder eingegeben. Die Platte wurde in der Simulation als eine monolithische Platte angenommen und nicht als Sandwichbauweise. Für diesen Aufbau wurde sich aufgrund der geringeren Komplexität des Modells und der fehlenden Materialeigenschaften entschieden, da im KAPA Line Datenblatt aus Anhang C nur die Materialeigenschaften der Sandwichbauweise und nicht die der einzelnen Materialien, Deckschicht und Kern, angegeben wurden.

Die Anregung der Platten wird im Rahmen einer Eigenfrequenzanalyse untersucht. Dabei werden die Eigenmoden der Platten im Frequenzbereich ab 0Hz bis zur 400. Eigenmode berechnet. Die Anzahl der Eigenmoden wurde gewählt, da diese für beide Platten die 1.000 Hz überschritten haben und die Modendichte mit zunehmender Frequenz zunimmt \cite{Cremer1996}. Im Konzeptentwurf wurde eine freihängende Lagerung ausgewählt. Aus diesem Grund wird die Platte in der Simulation ohne zusätzliche Lagerbedingungen modelliert.

Der Modellaufbau der Platte in COMSOL Multiphysics ist in Anhang F dargestellt und zeigt die Platte, welche mit einem Netz bespannt ist. In der Studie ist die gewünschte Anzahl der Eigenfrequenzen auf 400 eingestellt und die Berechnung startet bei \SI{0}{\hertz} und endet, wenn 400 Eigenmoden der Platte berechnet wurden. 


\subsection{Durchgeführte Simulationsstudien und Ergebnisse}

Die berechneten Eigenmoden der Platten bilden Muster, welche die Verschiebung der Platte bei den Eigenfrequenzen zeigen. Die Abbildungen \ref{fig:eigenmoden_07m} und \ref{fig:eigenmoden_1m} zeigen die Muster der simulierten Eigenmoden im Bereich der Frequenzen bei \SI{50}{\hertz}, \SI{150}{\hertz}, \SI{300}{\hertz}, \SI{500}{\hertz}, \SI{800}{\hertz} und \SI{1000}{\hertz} an.  

Die Legende, welche in den Abbildungen \ref{fig:eigenmoden_07m} und \ref{fig:eigenmoden_1m} zu sehen ist, zeigt an, dass die dunklen violetten Bereiche eine größere Verschiebung aufweisen. Je heller die Platte, desto weniger Verschiebung. 
\begin{figure}[H]
	\centering
	\includegraphics[angle=270,width=0.7\textwidth]{pictures/Eigenmoden_0.7m.png}
	\caption{Eigenmoden der KAPA Line Platte (\SI{0,7}{\meter} $\times$ \SI{0,44}{\meter})}
...
\begin{figure}[H]
	\centering
	\includegraphics[angle=270,width=0.6\textwidth]{pictures/Eigenmoden_1m.png}
	\caption{Eigenmoden der KAPA Line Platte (\SI{1}{\meter} $\times$ \SI{0,7}{\meter})}
	\label{fig:eigenmoden_1m}
\end{figure}

 

Die Plattenverschiebung bei den Eigenfrequenzen wird in der Auswertung genutzt, um die Positionen der Exciter zu bestimmen. Hierfür wurden die Daten der Simulation in Abhängigkeit zu einer durch die Mitte der Platte verlaufenden Referenzebene exportiert. Dabei wurde ein Raster über die Platte zur Auswertung der einzelnen Punkte gelegt. Anschließend wurden die Daten in einem Python-Skript anhand ihrer Verschiebung ausgewertet. Dabei wurden Punkte, an denen eine geringe Verschiebung auftritt, als gut und dort, wo eine hohe Verschiebung auftritt, als schlecht bewertet. Anhand dieser Bewertung wurde die ideale Position für zwei Exciter auf der Platte bestimmt. Das Skript berechnet für jede mögliche Kombination aus zwei Exciter-Positionen einen Score-Wert, der die Güte der Modenanregung quantifiziert. Die zehn besten Positionspaare werden tabellarisch ausgegeben, wobei neben dem Score auch die prozentuale Modenabdeckung und der minimale Abstand zwischen den Excitern angegeben werden.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{pictures/Skript_100x70.png}
	\caption{Ergebnisse Python-Skript mit den Top-10 Exciter-Positionspaaren für die 100 $\times$ 70\,cm Platte.}
	\label{fig:skript_100x70}
\end{figure}

Abbildung \ref{fig:skript_100x70} zeigt die Konsolenausgabe des Auswertungsskripts für die größere Platte. Die ermittelten optimalen Positionen liegen bei Exciter~1: $x = 50{,}0$\,mm, $z = 382{,}8$\,mm und Exciter~2: $x = 875{,}0$\,mm, $z = 65{,}6$\,mm (relativ zur linken unteren Ecke).

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{pictures/Skript_ergebnis_100x70.png}
	\caption{Visualisierung der Skript-Ergebnisse für die 100 $\times$ 70\,cm Platte.}
	\label{fig:skript_ergebnis_100x70}
\end{figure}

In Abbildung \ref{fig:skript_ergebnis_100x70} ist links die Score-Heatmap dargestellt, wobei helle Bereiche für einen hoher Score und eine günstige Positionen für Exciter~2 anzeigen. Die dunklen Bereiche kennzeichnen Knotenlinien der Eigenmoden, an denen eine Anregung vermieden werden sollte. 
Rechts ist die mittlere Modenaktivität über alle betrachteten Frequenzen dargestellt, wobei dunkle Bereiche eine geringe Verschiebung und somit geeignete Exciter-Positionen markieren.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{pictures/Skript_70x44.png}
	\caption{Ergebnisse Python-Skript mit den Top-10 Exciter-Positionspaaren für die 70 $\times$ 44\,cm Platte.}
	\label{fig:skript_70x44}
\end{figure}

Für die kleinere 70 $\times$ 44\,cm Platte (Abbildung \ref{fig:skript_70x44}) ergeben sich die optimalen Positionen bei Exciter~1: $x = 220{,}0$\,mm, $z = 251{,}6$\,mm und Exciter~2: $x = 330{,}0$\,mm, $z = 207{,}8$\,mm.

Die Ergebnisse der Heatmap-Analyse für die kleinere Platte, Abbildung \ref{fig:skript_ergebnis_70x44} zeigen ein charakteristisches Kreuzungsmuster der Knotenlinien in der Plattenmitte, welches bei der Exciter-Positionierung vermieden wurde. Die ermittelten Positionen liegen asymmetrisch zur Plattenmitte und ermöglichen eine breite Anregung der Plattenmoden.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{pictures/Skript_ergebni_70x44.png}
	\caption{Visualisierung der Skript-Ergebnisse für die 70 $\times$ 44\,cm Platte.}
	\label{fig:skript_ergebnis_70x44}
\end{figure}
...
\begin{itemize}
    \item Erzeugen und Wiedergabe eines Testsignals 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{pictures/Blockschaltbild LabVIEW.png}
...
\begin{itemize}
    \item $\Delta f$ : Frequenzauflösung des Spektrums in Hz
    \item $f_s$ : Abtastfrequenz in Hz
    \item $N$ : Anzahl der Abtastwerte pro Datenblock
\end{itemize}


\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{pictures/Whileschleife.png}
	\caption{Whileschleife des LabVIEW-Programms zur Vermessung des DML-Lautsprechers}
	\label{fig:whileschleife}
\end{figure}


In der Whileschleife Abbildung \ref{fig:whileschleife} wird das weiße Rauschen erzeugt, die Eingangssignale erfasst und verarbeitet sowie grafisch dargestellt. 

Das weiße Rauschen wird in der While-Schleife mit der Uniform White Noise Waveform VI generiert und über die Outputkarte NI PXI-6733, welche im Steckplatz 2 des DAQ NI PXI-1033 steckt, ausgegeben. Dafür wird die OutputBox NI SCC-68 verwendet, welche einen Cinch-Anschluss am Output 22 und 54 angeschlossen hat.
...
Aus den beiden Signalen lässt sich mittels Frequency Response VI die Übertragungsfunktion, sowie die zugehörige Kohärenz berechnen. Die zugehörigen komplexen Spektren werden als $X(k)$ (Referenzsignal) und $Y(k)$ (Mikrofonsignal) bezeichnet. Die mathematische Beschreibung der Übertragungsfunktion über den $H_1$-Schätzer sowie der Kohärenz erfolgt in den nachfolgenden Abschnitten.
...
\begin{itemize}
    \item $\bar{A}(k)$ : gemitteltes lineares Amplitudenspektrum in Pa
    \item $A_i(k)$ : lineares Amplitudenspektrum des $i$-ten Datenblocks in Pa
    \item $M$ : Anzahl der Mittelungen
...
\begin{itemize}
    \item $L_p(k)$ : Schalldruckpegel in dB SPL
    \item $\bar{A}(k)$ : gemitteltes lineares Amplitudenspektrum in Pa
    \item $p_0$ : Bezugsdruck $p_0 = 20 \cdot 10^{-6}$ Pa
    \item $k$ : Frequenzindex
\end{itemize}
...
\begin{itemize}
    \item $H_1(k)$ : $H_1$-Schätzer der Übertragungsfunktion
    \item $G_{xy}(k)$ : Kreuzspektrum zwischen Referenz- und Antwortsignal
    \item $G_{xx}(k)$ : Autospektrum des Referenzsignals
    \item $k$ : Frequenzindex
\end{itemize}
...
\begin{itemize}
    \item $G_{xy}(k)$ : gemitteltes Kreuzspektrum zwischen Referenz- und Antwortsignal
    \item $G_{xx}(k)$ : gemitteltes Autospektrum des Referenzsignals
    \item $X_i(k)$ : komplexes Spektrum des Referenzsignals im $i$-ten Block
    \item $X_i^*(k)$ : konjugiert komplexes Spektrum des Referenzsignals im $i$-ten Block
    \item $Y_i(k)$ : komplexes Spektrum des Antwortsignals im $i$-ten Block
    \item $M$ : Anzahl der gemittelten Blöcke
    \item $k$ : Frequenzindex
\end{itemize}
...
\begin{itemize}
    \item $H_{\mathrm{dB}}(k)$ : Betrag der Übertragungsfunktion in dB
    \item $|H_1(k)|$ : Betrag des $H_1$-Schätzers
    \item $k$ : Frequenzindex
\end{itemize}
...
\begin{itemize}
    \item $\gamma^2(k)$ : Kohärenzfunktion (Wertebereich 0 bis 1)
    \item $G_{xy}(k)$ : Kreuzspektrum zwischen Eingangs- und Ausgangssignal
    \item $G_{xx}(k)$ : Autospektrum des Referenzsignals
    \item $G_{yy}(k)$ : Autospektrum des Antwortsignals
    \item $k$ : Frequenzindex
\end{itemize}
...
Zur Untersuchung der akustischen Eigenschaften des DML-Systems wurden im Rahmen dieser Arbeit zwei Versuchsaufbauten umgesetzt. Um die Vergleichbarkeit der Messergebnisse zu gewährleisten, wurden die Einstellungen des Equalizers und des Audioverstärkers über alle Messungen hinweg konstant gehalten. Der Gain des Equalizers wurde auf den Wert 5 und die Lautstärke des Audioverstärkers auf den Wert 10 eingestellt. Eine Signalanpassung durch den Equalizer erfolgte nicht. Die Temperatur und Luftfeuchtigkeit waren während der Messungen konstant bei \SI{21}{\degreeCelsius} und 25\,\% Luftfeuchtigkeit.

Der erste Versuchsaufbau dient der Erfassung von den akustischen Eigenschaften des DML, hierbei werden zwei verschiedene Plattengrößen und jeweils unterschiedliche Exciter-Positionen vermessen. Bei den Plattengrößen handelt es sich um das Herstellermaß von 100 $\times$ 70\,cm und um das mit dem angepassten Seitenverhältnis von 70 $\times$ 44\,cm. Verglichen werden die Ergebnisse aus der Simulation, Kapitel 3.3.3 und die vom Hersteller angegebenen Positionen für die Exciter. Die exakten Exciterpositionen können den technischen Zeichnungen aus Anhang M--P entnommen werden. Zur Vermessung werden die Platten im Freifeldraum an Schnüren aufgehängt. Das Messmikrofon wird einen Meter vor der Plattenmitte positioniert. 
...
Im Folgenden werden die Ergebnisse der Schallfeldmessung des DML-Lautsprechers 70 $\times$ 44\,cm dargestellt und ausgewertet. Die Messung erfolgte im Freifeldraum der Hochschule mit einer horizontalen Winkelauflösung von \SI{15}{\degree} bei einem Messabstand von 1 m.
...
Abbildung~\ref{fig:schallfeld_spl_messung} zeigt die gemessene horizontale Richtcharakteristik des DML-Lautsprechers als Gesamtschalldruckpegel. Die Messwerte liegen im Bereich von \SI{68,4}{\decibel} bis \SI{80,5}{\decibel}, was einer Pegelschwankung von etwa \SI{12}{\decibel} entspricht. Das Abstrahlverhalten zeigt die für DML-Lautsprecher typische Dipolcharakteristik mit maximaler Abstrahlung in Richtung \SI{0}{\degree} zur Vorderseite und \SI{180}{\degree} an der Rückseite \cite{Harris2000,Bai2001}. Die minimalen Schalldruckpegel treten bei \SI{90}{\degree}, \SI{69,9}{\decibel} und \SI{270}{\degree}, \SI{68,4}{\decibel} auf, was auf die seitliche Auslöschung durch die gegenphasige Abstrahlung der Plattenvorder- und -rückseite zurückzuführen ist \cite{Harris2000}.



Die Richtcharakteristik wird zunächst als absoluter Schalldruckpegel und anschließend auf die Hauptabstrahlrichtung bei \SI{0}{\degree} normiert betrachtet, um sowohl das frequenzabhängige Abstrahlverhalten als auch die Richtwirkung des Lautsprechers zu beurteilen.
...
Abbildung~\ref{fig:schallfeld_absolut} zeigt die horizontale Richtcharakteristik des DML-Lautsprechers als absolute Schalldruckpegel. Im Tieftonbereich \SIrange{63}{125}{\hertz} sind die Pegel erwartungsgemäß niedrig und die Abstrahlung nahezu omnidirektional, da die Plattenabmessungen mit einer Plattendiagonale von ca.\ \SI{0{,}83}{\metre} weit unterhalb der akustischen Wellenlänge bei \SI{125}{\hertz} $\lambda \approx \SI{2{,}7}{\metre}$ liegen \cite{Moeser2005}.

Zwischen \SI{500}{\hertz} und \SI{2}{\kilo\hertz} steigen die abgestrahlten Pegel deutlich an und es formt sich die für DML-Lautsprecher typische Dipolcharakteristik mit Abstrahlung nach vorne und hinten \cite{Harris2000}. Die höchsten Absolutpegel treten bei ca.\ \SIrange{1}{2}{\kilo\hertz} auf, was auf den Bereich der Koinzidenzfrequenz hindeutet \cite{Cremer1996}. Leichte Asymmetrien im Richtmuster sind auf die bewusst exzentrische Exciter-Positionierung zurückzuführen.

Im Hochtonbereich \SIrange{4}{16}{\kilo\hertz} fragmentiert das Abstrahlmuster zunehmend. Die Interferenz vieler gleichzeitig schwingender, phasenverschobener Plattenbereiche erzeugt zahlreiche Nebenkeulen und Einbrüche \cite{Harris2000,Bai2001}. Die bei diesen Frequenzen sichtbare Links-Rechts-Asymmetrie spiegelt die Exciter-Position wider, kann aber teilweise auch durch die manuelle Winkelpositionierung der Platte bedingt sein.
...
Zwischen \SI{250}{\hertz} und \SI{1}{\kilo\hertz} bildet sich eine ausgeprägte Dipolform mit Einbrüchen von \SIrange{15}{25}{\deci\bel} bei \SI{90}{\degree} und \SI{270}{\degree}. Die Nullstellen sind erwartungsgemäß weniger scharf als bei einem idealen Punktdipol, da die Platte einen ausgedehnten Strahler mit endlicher Fläche und Kantenbeugung darstellt \cite{Cremer1996,Moeser2005}.

Ab \SI{2}{\kilo\hertz} verengt sich die Hauptkeule und es entstehen zunehmend Nebenkeulen. Bei \SI{8}{\kilo\hertz} und \SI{16}{\kilo\hertz} ist das Richtmuster stark fragmentiert \cite{Harris2000}. Die Winkelauflösung von \SI{15}{\degree} reicht in diesem Frequenzbereich möglicherweise nicht aus, um alle schmalen Keulen und Nullstellen vollständig aufzulösen.


\clearpage

Für die Frequenzgangmessung wurde zusätzlich ein konventioneller dynamischer Lautsprecher als Referenz vermessen, um die Ergebnisse der DML-Lautsprecher einordnen zu können. Verwendet wurde das Modell Bose ML3 BLK (Artikelnummer 125727), ein kompakter Fullrange-Lautsprecher.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{pictures/Bose_Referenz.png}
	\caption{Bose ML3 BLK Referenzlautsprecher}
	\label{fig:bose_referenz}
\end{figure}

Abbildung \ref{fig:bose_referenz} zeigt den Bose ML3 BLK Lautsprecher, der als Fullrange-System mit Hoch-, Mittel- und Tieftöner ausgestattet ist. Über den Vergleich mit einem konventionellen Lautsprecher lassen sich der Frequenzgang und das Abstrahlverhalten der DML-Varianten besser einordnen.
...
Abbildung \ref{fig:frontpanel_labview} zeigt das Frontpanel während der Vermessung der 70 $\times$ 44\,cm Platte mit den simulierten Exciterpositionen. Im Graphen oben links ist das Anregungssignal in Form eines weißen Rauschens zu sehen, oben rechts werden Mikrofon- und Referenzsignal dargestellt. In der Mitte sind die FRF-Übertragungsfunktion und die zugehörige Kohärenz aufgetragen, im unteren Bereich ist der Schalldruckpegel über der Frequenz dargestellt.

Die FRF beschreibt das Verhältnis zwischen Eingangs- und Ausgangssignal eines Systems im Frequenzbereich und gibt damit das frequenzabhängige Übertragungsverhalten wieder \cite{Mueller2001,Moeser2020}. Lokale Pegelmaxima in der FRF entsprechen Resonanzen, an denen die Platte durch Eigenmoden besonders effizient angeregt wird, während Einbrüche auf Knotenlinien hindeuten, an denen die Anregung durch den Exciter gering bleibt. Ein gleichmäßiger Verlauf weist demnach auf eine ausgewogene modale Energieverteilung hin, während starke Pegelschwankungen klangliche Verfärbungen erwarten lassen \cite{Harris2000}. Die Aussagekraft der Messungen lässt sich anhand der Kohärenz beurteilen, die während der gesamten Messreihe bei 0,9 bis nahe 1 lag und damit auf einen zuverlässigen, linear durch das Eingangssignal erklärbaren Messverlauf hinweist \cite{Mueller2001}.
...
\begin{figure}[H]
    ...
    \legend{70$\times$44 Hersteller, 70$\times$44 Simulation, 100$\times$70 Hersteller, 100$\times$70 Simulation, Bose Referenz}
    \end{semilogxaxis}
    \end{tikzpicture}
    \caption{FRF-Übertragungsfunktion im Vergleich aller Messungen}
    \label{fig:frequenzgang_vergleich}
\end{figure}


In Abbildung \ref{fig:frequenzgang_vergleich} sind die FRF-Übertragungsfunktionen der beiden DML-Platten in den Hersteller- und Skript-Positionen sowie der Bose-Referenz direkt gegenübergestellt. Die Linienzuordnung entspricht der Legende.


Im Tieftonbereich zwischen 25\,Hz und 150\,Hz fällt die 70$\times$44\,cm Platte mit Hersteller-Position von 98,5\,dB auf 82,9\,dB ab, was einer Differenz von 15,6\,dB entspricht. Die Skript-Position derselben Platte zeigt mit einem Abfall von 9,3\,dB einen deutlich geringeren Einbruch. Die 100$\times$70\,cm Platte bleibt in beiden Konfigurationen innerhalb von etwa 1\,dB konstant und ist damit im Tiefton merklich stabiler. Die Bose-Referenz weist in diesem Bereich einen Abfall von 6,7\,dB auf.

Zwischen 200\,Hz und 1\,kHz liegen alle DML-Konfigurationen in einem ähnlichen Pegelband zwischen 84 und 99\,dB. Die globalen Pegelmaxima treten bei 1.275\,Hz mit 97,7\,dB für die 70$\times$44\,cm Skript-Position und 98,1\,dB für die 100$\times$70\,cm Hersteller-Position auf. Im Bereich von 1\,kHz bis 8\,kHz zeigen alle DML-Kurven zahlreiche lokale Einbrüche und Resonanzen, die für die modale Anregung der Biegewellenplatten typisch sind \cite{Harris2000}. Die Bose-Referenz beginnt ab etwa 1,5\,kHz abzufallen und liegt bei 2.900\,Hz bereits 30\,dB unter ihrem Tieftonpegel.

Oberhalb von 8\,kHz halten die DML-Konfigurationen Pegel zwischen 60 und 85\,dB, während die Bose-Referenz auf 38 bis 55\,dB abfällt. Die DML-Varianten liegen damit durchgehend mindestens 15 bis 30\,dB über der Bose-Referenz, was die für DML charakteristische breitbandige Hochtonwiedergabe widerspiegelt \cite{Harris2000,Bai2001}.

Im direkten Vergleich der Positionierungen zeigt die 70$\times$44\,cm Platte einen um 6,3\,dB geringeren Tiefton-Einbruch in der Skript-Position. Bei der 100$\times$70\,cm Platte ist der Unterschied im Tiefton geringer, dafür treten in der Skript-Position markante Mittel- und Hochton-Einbrüche bei 1.775\,Hz und 7.775\,Hz mit Werten unter 70\,dB auf.

Auf Grundlage dieser Messergebnisse wird für den weiteren Verlauf der Arbeit das DML-System mit den Abmessungen 70$\times$44\,cm ausgewählt. Die Entscheidung stützt sich auf mehrere Beobachtungen aus der Messreihe.


Bei der 70$\times$44\,cm Platte ist der Unterschied zwischen Hersteller- und Skript-Position deutlich ausgeprägter als bei der 100$\times$70\,cm Platte. Während die Hersteller-Position im Tiefton einen Pegelabfall von 15,6\,dB aufweist, fällt dieser bei der Skript-Position mit 9,3\,dB merklich kleiner aus. Dieser Unterschied erlaubt eine klarere Bewertung der beiden Positionierungsstrategien und ist konsistent mit Ergebnissen aus der Literatur zu optimierter Exciter-Positionierung \cite{Zenker2019,Pueo2009}.

Hinsichtlich der Fullrange-Tauglichkeit zeigt die 70$\times$44\,cm Platte mit der Skript-Position einen relativ gleichmäßigen Frequenzgangverlauf ab ca.\ 60\,Hz mit Pegeln von 87--92\,dB. In Kombination mit einem Equalizer lässt sich daraus ein breitbandig nutzbares System ableiten. Der im Vergleich zur 100$\times$70\,cm Platte geringere Tieftonpegel unterhalb von 80\,Hz kann in der Praxis durch raumakustische Effekte oder einen ergänzenden Subwoofer ausgeglichen werden.
...
Abbildung~\ref{fig:schalldruckpegel_spektrum} zeigt das gemessene Schalldruckpegel-Spektrum der vier DML-Konfigurationen sowie der Bose-Referenz von 25\,Hz bis 20\,kHz. Die Linienzuordnung entspricht der Legende.

Im Tieftonbereich von 25\,Hz bis 200\,Hz erzielt die 100$\times$70\,cm Platte mit Hersteller-Position mit 45,0 bis 54,3\,dB die höchsten Pegel. Die 70$\times$44\,cm Platte liegt in der Hersteller-Position mit 40,0 bis 48,3\,dB am niedrigsten, in der Skript-Position mit 42,0 bis 52,5\,dB im Mittel etwa 3\,dB höher. Die 100$\times$70\,cm Platte fällt in der Skript-Position auf 42,0 bis 50,3\,dB zurück und damit auf das Niveau der kleineren Platte.

Im unteren Mittelton zwischen 400\,Hz und 1.500\,Hz bildet die 70$\times$44\,cm Platte mit Skript-Position mit Werten von 53,8 bis 64,7\,dB die lauteste Konfiguration und übertrifft dort teilweise die größere Platte. Während bei der 70$\times$44\,cm Platte die Skript-Position im Mittel etwa 4,5\,dB über der Hersteller-Position liegt, kehrt sich das Verhältnis bei der 100$\times$70\,cm Platte um: Dort liegt die Hersteller-Position mit etwa 58,6\,dB rund 3\,dB über der Skript-Position bei 55,4\,dB.

Oberhalb von 5\,kHz liegen die DML-Konfigurationen in einem Pegelband zwischen 33 und 57\,dB. Die Bose-Referenz überschneidet sich zwischen 5 und 12\,kHz noch mit 25 bis 50\,dB teilweise mit den DML-Pegeln, fällt aber oberhalb von 14\,kHz auf 19,6 bis 29,2\,dB ab und liegt damit deutlich unter den DML-Varianten.

Über den gesamten Frequenzbereich beträgt die Pegelschwankung der DML-Konfigurationen 25 bis 30\,dB, die der Bose-Referenz dagegen 38\,dB. Die geringere Streubreite der DML-Systeme lässt sich auf die breitbandige Abstrahlung der Biegewellenplatten zurückführen \cite{Harris2000,Bai2001}.

\clearpage

\section{Validierung und Vergleich der Ergebnisse}

Um die Aussagekraft der simulationsgestützten Exciter-Positionierung zu bewerten, wird im Folgenden untersucht, inwieweit die in der FEM-Simulation berechneten Eigenfrequenzen und Modenformen mit den im Frequenzgang sichtbaren Resonanzen und Einbrüchen korrespondieren. Zusätzlich werden die DML-Konfigurationen mit dem Bose-Referenzlautsprecher verglichen und die gewonnene Richtcharakteristik gegen die theoretische Erwartung gestellt.

\subsection{Vergleich der simulierten Eigenfrequenzen mit gemessenen Resonanzen}

Die Eigenfrequenzanalyse in COMSOL berechnet für die 70$\times$44\,cm KAPA Line Platte 400 Eigenmoden ab 0\,Hz, deren höchste Frequenzen oberhalb von 1\,kHz liegen. Aus diesem Modenspektrum sind in Abbildung~\ref{fig:eigenmoden_07m} sechs Modenformen bei 50, 150, 300, 500, 800 und 1000\,Hz dargestellt. Eine simulierte Eigenfrequenz erzeugt im FRF entweder ein lokales Maximum, sofern der Exciter die zugehörige Mode anregt, oder ein lokales Minimum, sofern er auf einer Knotenlinie der Mode liegt \cite{Cremer1996}. Tabelle~\ref{tab:eigenfrequenz_vergleich} fasst die Zuordnung zwischen den Eigenfrequenzen und dem FRF-Verhalten der 70$\times$44\,cm Platte zusammen.

\begin{table}[H]
\centering
\caption{Zuordnung simulierter Eigenfrequenzen zu Merkmalen im FRF-Spektrum der 70$\times$44\,cm Platte}
\label{tab:eigenfrequenz_vergleich}
\begin{tabularx}{\textwidth}{|l|X|X|X|}
\hline
\textbf{Sim.\ Eigenfreq.} & \textbf{FRF Hersteller} & \textbf{FRF Skript} & \textbf{Bewertung} \\
\hline
ca.\ 50\,Hz & nicht erfasst & nicht erfasst & außerhalb der Datenpunkte \\
\hline
ca.\ 150\,Hz & 82,9\,dB & 81,7\,dB & Knotenlinie nahe beider Positionen \\
\hline
ca.\ 300\,Hz & 97,4\,dB (Maximum) & 88,2\,dB & Maximum bei Hersteller \\
\hline
ca.\ 500\,Hz & 98,8\,dB (Maximum) & 95,6\,dB (Maximum) & Maximum bei beiden \\
\hline
ca.\ 800\,Hz & 86,5\,dB & 85,0\,dB & Knotenlinie nahe beider Positionen \\
\hline
ca.\ 1000\,Hz & 92,7\,dB & 89,8\,dB & moderate Pegel, kein klares Merkmal \\
\hline
\end{tabularx}
\end{table}

Die Modenformen bei 300\,Hz und 500\,Hz korrelieren mit deutlichen Pegelmaxima, während bei 150\,Hz und 800\,Hz beide Exciter-Positionen lokale Minima zeigen, was auf Knotenlinien nahe beider Positionen hindeutet. Diese Beobachtung wird in Abschnitt~\ref{subsec:knotenlinien} vertieft. Oberhalb von 2\,kHz nimmt die Modendichte stark zu, sodass einzelne Eigenmoden nicht mehr eindeutig zugeordnet werden können \cite{Cremer1996}. Die Abweichungen zwischen simulierten und gemessenen Frequenzen lassen sich auf die monolithische Ersatzmodellierung der Sandwichplatte, die Übernahme des E-Moduls von KAPA Graph für KAPA Line sowie die Vernachlässigung des Exciters im FEM-Modell zurückführen.

\subsection{Korrelation zwischen Knotenlinien und Frequenzgang-Einbrüchen}
\label{subsec:knotenlinien}

Im Tieftonbereich zeigt sich der Effekt der Knotenlinien-Anregung am deutlichsten. Bei der 70$\times$44\,cm Hersteller-Position fällt der Pegel von 98,5\,dB bei 25\,Hz auf 82,9\,dB bei 150\,Hz und steigt bei 275\,Hz wieder auf 97,4\,dB an. Da die Skript-Position bei 150\,Hz mit 81,7\,dB einen vergleichbaren Wert erreicht, ist von einer Knotenlinie der 150\,Hz-Mode in der Nähe beider Exciter-Positionen auszugehen. Der Unterschied liegt in den benachbarten Moden: Die Skript-Position bleibt mit 91,0\,dB bei 25\,Hz und 88,2\,dB bei 275\,Hz im Mittel rund 7\,dB niedriger, sodass der Tiefton-Einbruch dort mit 9,3\,dB statt 15,6\,dB merklich kleiner ausfällt.

Die Heatmap-Analyse in Abbildung~\ref{fig:skript_ergebnis_70x44} zeigt, dass die Skript-Positionen asymmetrisch zur Plattenmitte liegen und das Kreuzungsmuster der Knotenlinien gezielt vermeiden. Allgemein gilt: Bei hohen Score-Werten der Simulation, wenn beide Exciter außerhalb von Knotenlinien liegen, treten hohe FRF-Pegel auf; bei niedrigen Score-Werten messbare Einbrüche. Diese Korrelation ist im Bereich von 50--2000\,Hz am deutlichsten, da hier die Modenstruktur noch überschaubar bleibt.

\subsection{Bewertung der Simulationsmethodik}

\textbf{Stärken:}
\begin{itemize}
    \item Die monolithische Ersatzmodellierung liefert Eigenfrequenzen, die im Tiefton- und Mittenbereich gut mit Merkmalen des FRF übereinstimmen.
    \item Die Score-basierte Optimierung führt zu einem gleichmäßigeren Tiefton-Frequenzgang.
    \item Die Vorhersage von Pegelminima durch Knotenlinien-Analyse stimmt qualitativ mit den Messergebnissen überein.
\end{itemize}

\textbf{Grenzen:}
\begin{itemize}
    \item Oberhalb von 2\,kHz nimmt die Modendichte so stark zu, dass nur statistische Aussagen möglich sind \cite{Cremer1996}.
    \item Der Exciter wurde nicht als Masse-Feder-System modelliert; seine Masse und lokale Steifigkeit beeinflussen die Modenstruktur.
    \item Die Sandwichstruktur und ihre frequenzabhängige Dämpfung wird durch das monolithische Modell nur näherungsweise erfasst.
    \item Die Übernahme des E-Moduls von KAPA Graph für KAPA Line kann zu systematischen Frequenzverschiebungen führen.
\end{itemize}

\subsection{Vergleich mit dem Bose-Referenzlautsprecher}

Im Tiefton bewegt sich die Bose-Referenz mit 43,0 bis 51,8\,dB im Mittelfeld der DML-Konfigurationen; die für DML erwartete eingeschränkte Tieftonwiedergabe \cite{Harris2000} fällt erst unterhalb von 80\,Hz deutlich gegenüber Bose aus.

Oberhalb von 2\,kHz fällt die Bose-Referenz im FRF um etwa 30\,dB ab, während die DML-Konfigurationen Pegel zwischen 60 und 85\,dB halten. Im SPL-Spektrum erreicht Bose oberhalb von 14\,kHz nur 19,6 bis 29,2\,dB gegenüber 33 bis 57\,dB der DML-Varianten. Diese breitbandige Hochtonabstrahlung ergibt sich aus der Vielzahl gleichzeitig schwingender, phasenverschobener Plattenbereiche \cite{Harris2000,Bai2001}.

Die Pegelschwankung der DML-Konfigurationen liegt mit 25 bis 30\,dB unter der Schwankung der Bose-Referenz von 38\,dB. Während Bose typische Kolbenstrahler-Merkmale wie Tiefton-Hochpass, Mitten-Plateau und kontinuierlichen Hochtonabfall zeigt, weisen die DML-Konfigurationen die für Plattenresonatoren typische, modenbedingte Welligkeit auf \cite{Cremer1996}.

\subsection{Validierung der Richtcharakteristik}

Die in Abbildung~\ref{fig:schallfeld_normiert} dargestellte horizontale Richtcharakteristik der 70$\times$44\,cm Platte stimmt mit den theoretischen Erwartungen überein. Bei 63\,Hz schwankt der Pegel über alle Winkel um nur etwa 9\,dB, was einer nahezu omnidirektionalen Abstrahlung entspricht. Die Wellenlänge $\lambda = 343/63 \approx 5{,}4$\,m ist hier ein Vielfaches der Plattendiagonale von 0,83\,m, sodass die Platte als kompakte Quelle wirkt \cite{Moeser2005}.

Zwischen 250\,Hz und 1\,kHz bildet sich die für freihängende DML-Platten charakteristische Dipolform mit Maxima bei $0^\circ$ und $180^\circ$ sowie Einbrüchen von 15 bis 25\,dB bei $90^\circ$ und $270^\circ$ aus. Dieses Verhalten resultiert aus der gegenphasigen Abstrahlung der Plattenseiten \cite{Harris2000}; aufgrund der endlichen Plattenfläche sind die Nullstellen weniger scharf als bei einem idealen Punktdipol \cite{Cremer1996}. Oberhalb von 2\,kHz fragmentiert das Abstrahlmuster zunehmend, was mit der phasenverschobenen Abstrahlung vieler gleichzeitig schwingender Plattenbereiche übereinstimmt \cite{Harris2000,Bai2001}.

\subsection{Diskussion möglicher Fehlerquellen}

Die Winkelpositionierung bei der Richtcharakteristik messung der Platte, erfolgte manuell in \SI{15}{\degree}-Schritten mit hilfe von Laserprojektion. Abweichungen von wenigen Grad können bei hohen Frequenzen die schmalen Keulen verfälschen.

Der Messabstand von \SI{1}{\metre} erfüllt die Fraunhofer-Fernfeldbedingung $r > 2D^2/\lambda$ \cite{Lerch2009} für die größte Plattenabmessung $D = \SI{0{,}7}{\metre}$ nur bis zu einer Grenzfrequenz von $f < c/(2D^2) \approx \SI{350}{\hertz}$. Bereits ab dem unteren Mittelton befinden sich die Messungen damit im geometrischen Nahfeld der Platte; bei \SI{4}{\kilo\hertz} wäre ein Abstand von ca.\ \SI{11}{\metre}, bei \SI{16}{\kilo\hertz} von ca.\ \SI{46}{\metre} erforderlich. Im Nahfeld gemessene Pegel und Richtmuster können daher von der tatsächlichen Fernfeldabstrahlung abweichen.

Im Freifeldraum befanden sich während der Messung zudem Stativkonstruktionen, Messgeräte, Gitterroste am Boden sowie ein Verbrennungsmotor auf einem Gestell, wie in Abbildung~\ref{fig:messaufbau_freifeldraum} zu erkennen ist. Diese können im Hochtonbereich als reflektierende Flächen wirken.

Trotz dieser Fehlerquellen sind die Messergebnisse aussagekräftig, da sie die charakteristischen Eigenschaften von DML-Lautsprechern -- Dipolcharakteristik, modenbedingte Welligkeit und ausgeprägte Hochtonabstrahlung gegenüber konventionellen Lautsprechern -- konsistent wiedergeben.
